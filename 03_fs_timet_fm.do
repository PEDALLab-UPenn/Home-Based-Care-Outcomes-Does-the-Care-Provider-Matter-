...
* Subsample: Female care recipients                                            *
...
********************************************************************************
	...
	* Run with female care recipients
	keep if female==1
...
sa "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_fm.dta", replace
...
use "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_fm.dta", clear
...
mlogit multi_nomedhh sib_cnt numkids_all $controls [pw=rwtresp] if s_ltc_1adl_50==1, vce (cluster stateid) base(1)	
	...
	merge 1:1 hhidpn wave using "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_fm.dta", nogen
...
foreach i of numlist `listoftables' {  /*start i loop */
    ...
    estout pro`i'_* using "$output/female/Appendix Table 2 FS.tex", cells(b(fmt(3) star) se(par)) ///
