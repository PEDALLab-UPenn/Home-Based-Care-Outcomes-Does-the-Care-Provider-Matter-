...
* Subsample: Care recipients without spouses                                   *
...
********************************************************************************
	...
	eststo clear
	use "~/IC_Aim2_FNL/createddata/regressiondata_fnl.dta", clear
	
	* Run with care recipients with no spouses
	drop if marital_stat==1	
...
sa "~/IC_Aim2_FNL/createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_nosp.dta", replace
...
use "~/IC_Aim2_FNL/createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_nosp.dta", clear
...
mlogit multi_nomedhh sib_cnt numkids_all $controls [pw=rwtresp] if s_ltc_1adl_50==1, vce (cluster stateid) base(1)	
	...
	merge 1:1 hhidpn wave using "~/IC_Aim2_FNL/createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_nosp.dta", nogen
...
foreach i of numlist `listoftables' {  /*start i loop */
    ...
    estout pro`i'_* using "$output/nosp/Appendix Table 2 FS.tex", cells(b(fmt(3) star) se(par)) ///
