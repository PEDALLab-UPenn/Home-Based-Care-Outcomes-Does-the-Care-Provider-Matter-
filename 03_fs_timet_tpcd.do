...
* Subsample: Top coded large families at the 90th percentile                   *
...
********************************************************************************
	...
	* Top code sib and kids count 
	ta sib_cnt if s_ltc_1adl_50==1, m
	ta numkids_all if s_ltc_1adl_50==1, m
	
	replace sib_cnt=7 if sib_cnt>7 & !mi(sib_cnt)
	replace numkids_all=6 if numkids_all>6 & !mi(numkids_all)
...
sa "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_tpcd.dta", replace
...
use "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_tpcd.dta", clear
...
mlogit multi_nomedhh sib_cnt numkids_all $controls [pw=rwtresp] if s_ltc_1adl_50==1, vce (cluster stateid) base(1)	
	...
	merge 1:1 hhidpn wave using "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_tpcd.dta", nogen
...
foreach i of numlist `listoftables' {  /*start i loop */
    ...
    estout pro`i'_* using "$output/tpcd/Appendix Table 2 FS.tex", cells(b(fmt(3) star) se(par)) ///
