...
INPUT: 	$createddata/regressiondata_fnl_multi_nomedhh_sondausisiv.dta

OUTPUT: Main Table 8 Panel A. Sisters, sons, and daughters as instruments
...
log using "$log/2sri_main_table8A_sondausisiv.log", replace
	...
	* First bootstrap to get standard errors; save in matrix
		...
		* First stage calc										  										
			di "BEGIN CALCULATION OF MLOGIT"
				qui mlogit multi_nomedhh numson numdaugh sib_sis_cnt `controls`counter'' [pw=rwtresp_bs] if tempsample, vce (cluster stateid) base(1) iter(300)				
				di "qui mlogit multi_nomedhh numson numdaugh sib_sis_cnt `controls`counter'' [pw=rwtresp_bs] if tempsample, vce (cluster stateid) base(1) iter(300)"											
...
* Next run main results and access bootstrapped standard error from matrix
	...
	foreach i of numlist `listoftables' {  /*start i loop */
		...
		* First stage calc: mlogit																
		qui mlogit multi_nomedhh numson numdaugh sib_sis_cnt `controls`count'' [pw=rwtresp] if tempsample, vce (cluster stateid) base(1)					
		di "qui mlogit multi_nomedhh numson numdaugh sib_sis_cnt `controls`count'' [pw=rwtresp] if tempsample, vce (cluster stateid) base(1)"
...
loc j = `i'	
	estout tsrib`i'_* using "$output/Table 8A.tex", cells(none) ///
