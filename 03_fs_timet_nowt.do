...
sa "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_nowt.dta", replace
...
use "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_nowt.dta", clear
...
* FIRST STAGE * 
mlogit multi_nomedhh sib_cnt numkids_all $controls if s_ltc_1adl_50==1, vce (cluster stateid) base(1)	

	g s1_sample = e(sample)
	keep hhidpn wave s1_sample
	merge 1:1 hhidpn wave using "$createddata/regressiondata_fnl_multi_nomedhh_sibkidsiv_nowt.dta", nogen
			
			
* SECOND STAGE *
keep if s1_sample==1

	probit depression_binary i.multi_nomedhh $controls if s_ltc_1adl_50==1, vce(cluster stateid)
		cap drop tempsample
		cap qui gen tempsample=e(sample)

	mlogit multi_nomedhh sib_cnt numkids_all $controls if tempsample, vce (cluster stateid) base(1)
	...
	probit depression_binary i.multi_nomedhh Xuhat_2 Xuhat_3 $controls if tempsample, vce(cluster stateid)
...
foreach i of numlist `listoftables' {  /*start i loop */
    ...
    foreach var of local outcomes`i' {  /*start var loop*/				
		...
		* Set sample for first stage calc
			...
			cap qui probit `var' i.multi_nomedhh `controls`count'', vce(cluster stateid)		
			di "cap qui probit `var' i.multi_nomedhh `controls`count'', vce(cluster stateid)"
			cap drop tempsample
			cap qui gen tempsample=e(sample)
			
	
		* Run FS regression 	
	        eststo pro`i'_`count': mlogit multi_nomedhh sib_cnt numkids_all `controls`count'' if tempsample, vce (cluster stateid) base(1)
    ...
    estout pro`i'_* using "$output/nowt/Appendix Table 2 FS.tex", cells(b(fmt(3) star) se(par)) ///
