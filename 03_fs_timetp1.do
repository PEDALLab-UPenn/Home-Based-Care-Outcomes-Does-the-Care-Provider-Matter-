...
* Robustness check: outcomes measured at time t+1                              *
...
********************************************************************************
	...
	eststo clear
	use "$createddata/timep1/regressiondata_fnl.dta", clear
	...
	egen sample = rowmiss(multi_nomedhh sib_cnt numkids_all ///
					  ...
					  depression_lead rshlt demented ever_stroke rarelig bmi_cat drink_cat) 
...
sa "$createddata/timep1/regressiondata_fnl_multi_nomedhh_sibkidsiv.dta", replace
...
mlogit multi_nomedhh sib_cnt numkids_all $controls [pw=rwtresp] if s_ltc_1adl_50==1, vce (cluster stateid) base(1)	
	...
	merge 1:1 hhidpn wave using "$createddata/timep1/regressiondata_fnl_multi_nomedhh_sibkidsiv.dta", nogen
			
			
* SECOND STAGE *
keep if s1_sample==1

	probit depression_lead i.multi_nomedhh $controls [pw=rwtresp] if s_ltc_1adl_50==1, vce(cluster stateid)
		cap drop tempsample
	...
	probit depression_lead i.multi_nomedhh Xuhat_2 Xuhat_3 $controls [pw=rwtresp] if tempsample, vce(cluster stateid)
...
********************************************************************************
    ...
    loc controls1 "educ_m_yrs ib(2).race ib(2).race#c.educ_m_yrs ib(1).hispanic ib(1).female ib(6).age_band ib(4).marital_stat ib(0).first_marriage ib(0).us_born ib(1).educ_cat ib(1).networth_pctl adl_tot_any iadl_tot_any ib(0).medicare ib(1).medicaid ib(0).priv_insurance ib(0).ltc_insur ib(1).demented ib(1).ever_stroke ib(4).rarelig i.stateid i.wave i.bmi_cat i.drink_cat"  		
	...
	local outcomes1    "shealth_lead mobility_lead depression_lead affect_lead" 
...
foreach i of numlist `listoftables' {  /*start i loop */
    ...
    estout pro`i'_* using "$output/timep1/Appendix Table 2 FS.tex", cells(b(fmt(3) star) se(par)) ///
